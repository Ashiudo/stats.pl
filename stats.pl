...
sub ScoresTSN {
    ...
    foreach( lc( $league ) ) {
        if( /^(nhl|nba|nfl|mlb|cfl)$/ ) {
            #$url .= "GET/urn:tsn:$1:scoreboard?type=json"; }
...
sub ScoresESPN {
    ...
    foreach( lc( $league ) ) {
        if( /^epl$/ ) {
            $url = 'http://soccernet.espn.go.com/scores/_/league/eng.1/?'; }
        elsif( /^mls$/ ) {
            $url = 'http://soccernet.espn.go.com/scores/_/league/usa.1/?'; }
        elsif( /^(seriea|italy)$/ ) {
            $url = 'http://soccernet.espn.go.com/scores/_/league/ita.1/?'; }
        elsif( /^(laliga|spain)$/ ) {
            $url = 'http://soccernet.espn.go.com/scores/_/league/esp.1/?'; }
        elsif( /^(bundesliga|germany)$/ ) {
            $url = 'http://soccernet.espn.go.com/scores/_/league/ger.1/?'; }
        elsif( /^(ligue1|france)$/ ) {
            $url = 'http://soccernet.espn.go.com/scores/_/league/fra.1/?'; }
        elsif( /^(eredivisie|dutch|holland|netherlands)$/ ) {
            $url = 'http://soccernet.espn.go.com/scores/_/league/ned.1/?'; }
        elsif( /^(uefa|euro)$/ ) {
            $url = 'http://soccernet.espn.go.com/scores/_/league/UEFA.EURO/?'; }
        else {
            return }
    }
    ...
    my $olddate = $date;
    $date = GetDate( $date, '%Y%m%d' );
    $url .= "date=$date" if( $date !~ /invalid/ );

    my $data = download( $url );
    my( $curdate ) = $data =~ /<li id="currentDate".*?>(.*?)</s;
    my( $timezone ) = $data =~ /All kick-off times in ([A-Za-z]+)/s;
    #($data) = ( $data =~ /<div class="live-tables"(.*?)\n/s );
    my @games = $data =~ /div class="scores-top">(.*?)<\/div>/sg;
    return 'No games found' if( !@games || GetDate( $curdate, '%s' ) != GetDate( $olddate, '%s' ) );
    #print $#games, " games found\n";

    my @ret;
    foreach( @games ) {
        #print "$_\n\n";
        my( $status ) = /"status">(.*?)</s;
        my( $hteam, $score, $ateam ) = /a href.*?>(.*?)</sg;
        my( $hscore, $ascore );
        if( $search && ( $search ne '*' ) ) {
            next if ( "$hteam $ateam" !~ /\Q$search\E/i );
        }
        ($hscore, $ascore ) = ( $1, "$2 " ) if( $score =~ /([\d()]+).*?([\d()]+)/ );
        if( $hscore eq '' ) {

            $hscore = 'vs';
            $status =~ s/,//g;
            my $tmptime = GetDate( "$status $timezone", '%l:%M%p ET' );
            $status = $tmptime if( $status ne 'Postp' && $tmptime !~ /invalid/ );
            $status =~ s/(?:^ |\n)//g;
        }
        push @ret, "$hteam $hscore $ateam $ascore( $status )";
    }
    return @ret;
} #GetSscoresESPN( )
...
sub Scores {
    ...
    return @tmpscores if( $tmpscores[0] ne 'no games found' );

    print "Scores league:$league search:$search date:$date\n" if( DEBUG );

    if ( $league =~ /^nc[a]*([bf])$/ ) {
        if( $1 eq 'f' ) {
            $football = 1;
            $url = 'http://rivals.yahoo.com/ncaa/football/scoreboard?';
        } else {
        ...
        my $filter;
        $search = '*';
        foreach( lc( $tmpsearch ) ) {
            if(/^\*$/) { $filter = 'ncaa'; }
            elsif(/top$/) {
                $filter = 'top25';
                $date = ''; }
            elsif(/top25/) { $filter = 'top25'; }
            elsif(/sec/) { $filter = 'sec'; }
            elsif(/fbs/) { $filter = 'all'; }
            elsif(/acc/) { $filter = 'acc'; }
            elsif(/sun$/) {
                $filter = 'sbelt';
                $date = ''; }
            elsif(/sunb/) { $filter = 'sbelt'; }
            elsif(/mvc/) { $filter = 'mvc'; }
            elsif(/cusa/) { $filter = 'cusa'; }
            elsif(/wac/) { $filter = 'wac'; }
            elsif(/big/) {
                if( length( $tmpsearch ) > 3 ) {
                    $tmpsearch = substr( $tmpsearch, 3 );
                } else {
                    return if( !$date );
                    $tmpsearch = $date;
                    $date = '';
                ...
                foreach($tmpsearch) {
                    if( /10/ ) { $filter = 'big10'; }
                    elsif( /ten/i ) { $filter = 'big10'; }
                    elsif( /12/i ) { $filter = 'big12'; }
                    elsif( /east/i ) { $filter = 'bige'; }
                    elsif( /sky/i ) { $filter = 'bsky'; }
                    elsif( /south/i ) { $filter = 'bsou'; }
                    }
                }
            elsif(/pac/) { $filter = 'pac12'; }
            elsif(/fcs/) { $filter = 'iaa_all'; }
            elsif(/southland/) { $filter = 'sland'; }
            elsif(/pio/) { $filter = 'pio'; }
            elsif(/nec/) { $filter = 'nec'; }
            elsif(/gwest/) { $filter = 'gwest'; }
            elsif(/greatwest/) { $filter = 'gwest'; }
            elsif(/great$/) {
                $filter = 'gwest';
                $date = ''; }
            elsif(/caa/) { $filter = 'caa'; }
            elsif(/pl/) { $filter = 'patr'; }
            elsif(/ovc/) { $filter = 'ovc'; }
            elsif(/mvc/) { $filter = 'mvc'; }
            elsif(/ivy/) { $filter = 'ivy'; }
            elsif(/swac/) { $filter = 'swac'; }
            elsif(/south/) { $filter = 'south'; }
            else { $search = $tmpsearch; }
        }
        $url .= "c=$filter&" if( $filter );
    } elsif ( $league eq 'nfl' ) {
        $football = 1;
        ...
        foreach( $league ) {
            if( /^(nhl|nba|mlb)$/ ) {
                $url = "http://sports.yahoo.com/$1/scoreboard?"; }
            elsif( /^(wj|i+hf|wh|wor|cfl|ahl|mlb)/ ) {
                return ScoresTSN( $league, $search, $date ); }
            else {
                return ScoresESPN( $league, $search, $date );
            }
        }
        return unless( $url );
    }
    ...
    my @days;
    if( $football ) {
        $data =~ s/<tr class="?ysptblthbody2"?>/!!!fbmark!!!<tr class="ysptblthbody2">/sg ;
        $data .= '!!!fbmark!!!';
        @days = ( $data =~ /<tr class="?ysptblthbody2"?>(.*?)!!!fbmark!!!/sg );
    } else {
        $days[0] = $data;
    }
    ...
    OUTER: foreach( @days ) {
        ...
        my @games = ( /<table[^>]+?class="?(?:scores|ysptblclbg[0-9])"?>(.*?)<\/table>/sg );
        my $day = join( '', /[\s]*(...).*?( ...).*?( [0-9]+)/ );

        INNER: for( my $i = 0; $i < @games; $i++ ) {

            my @teams = ( $games[$i] =~ /<td .*?align="?left"? class="?yspscores team.*?<b>[\s]*(.*?)<\/td/sg );
            @teams = ( $games[$i] =~ /<td .*?align="?left"? class="?yspscores.*?<b>[\s]*(.*?)<\/td/sg ) if( @teams != 2 );
            ...
            if( $search && ( $search ne '*' ) ) {
                if( $league eq 'nhl' ) {
                    foreach( @teams ) { $_ = FindTeam($_) }
                    my $tmatch = "@teams " . FindTeam($teams[0]) . ' ' . FindTeam($teams[1]);
                    next INNER if ( $tmatch !~ /\Q$search\E/i );
                } else {
                    if ( $teams[0] !~ /\Q$search\E/i ) {
                        next INNER if ( $teams[1] !~ /\Q$search\E/i );
                    }
                }
            } elsif( $league eq 'nhl' ) {
                foreach( @teams ) { $_ = FindTeam($_) }
            ...
            my @scores = ( $games[$i] =~ /<span class="?yspscores"?>(.*?)<\/span>/sg );
            foreach( @scores ) {
                s/[\s]*<.*?>[\s]*//sg;
            }

            if( $search ) {
                if( @scores < 3 || $scores[1] =~ /E[DS]?T/ ) {
                    push @ret,
                        !$football ?    "$teams[0] @ $teams[1] ( $scores[0] $scores[1] )"
                        :               "$teams[0] @ $teams[1] ( $day $scores[0] $scores[1] )";
                } else {
                    if( $league eq 'mlb' ) {
                        ($scores[1]) = $games[$i] =~ /<td .*?class="yspscores" align="left">[\s]*(.*?)\n/s;
                        $scores[2] = $scores[3];
                    }
                    ($scores[2],$scores[1]) = ($scores[1],$scores[2]) if( $scores[2] =~ /final/i );
                    push @ret, "$teams[0] $scores[0] $teams[1] $scores[2] ( $scores[1] $scores[3] )";
                }
            } else {
                foreach( @scores ) {
                    next INNER if( /E[DS]?T|final/i );
                }
                if( $league eq 'mlb' ) {
                    ($scores[1]) = $games[$i] =~ /<td .*?class="yspscores" align="left">[\s]*(.*?)\n/s;
                    $scores[2] = $scores[3];
    ...
    return 'No results found.' if( @ret == 0 );
    return @ret;
} #Scores( )
